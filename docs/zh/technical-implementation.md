...
11. 任一步失败，`mark_failed()` 保存错误信息，并把 job 标记为 `failed`。
12. job 结束后标记为 `completed` / `failed`，并清理队列临时音频文件。

这套上传流程和实时流程共用同一批核心服务，所以上传会议也能得到 transcript、translation、analysis、summary、action items 和历史记录。队列临时音频只用于任务处理，和用户主动开启的原始音频留存是两套路径。
...
4. 上传任务使用 SQLite 持久队列；FastAPI 默认启动内置 worker，也可用 `tools/run_upload_worker.py` 独立处理。复杂重试 backoff 和 attempt 治理仍是后续工作。
