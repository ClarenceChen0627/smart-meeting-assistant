...
6. 把上传音频写入 `UPLOAD_QUEUE_DIR` 下的临时 payload 文件。
7. 在 SQLite `upload_jobs` 表中创建 `queued` 任务。
8. 默认由内置 upload worker 自动 claim 并处理；也可以用 `tools/run_upload_worker.py` 单独运行 worker。
9. 立即返回当前会议记录。
...
11. 单次 job attempt 内部会对 ASR、analysis、summary 的 `RuntimeError` 即时重试一次。
12. 如果整条 job attempt 仍失败，队列记录 `last_error`，按指数退避设置 `next_run_at`，并在未耗尽 `UPLOAD_QUEUE_MAX_ATTEMPTS` 时保持 meeting 为 `processing`。
13. attempt 耗尽后才 `mark_failed()` 保存终态错误，并把 job 标记为 `failed`。
14. job 成功或终态失败后清理队列临时音频文件；可重试失败不会清理 payload。

这套上传流程和实时流程共用同一批核心服务，所以上传会议也能得到 transcript、translation、analysis、summary、action items 和历史记录。队列临时音频只用于任务处理，和用户主动开启的原始音频留存是两套路径。服务启动时会释放超过 `UPLOAD_QUEUE_PROCESSING_TIMEOUT_SECONDS` 的 stale processing claim；没有 active queue job 的 processing upload 会被标记为 interrupted。队列 payload 文件缺失属于不可恢复错误，会直接让 job 和 meeting 进入 failed。
...
`AuditLogService` 同样使用这个 SQLite 文件，并创建 `audit_events` 表记录成功提交的人工编辑。每条事件包含 `scope`、`meeting_id`、`entity_type`、`entity_id`、`action`、`field_path`、JSON `before` / `after` 快照、JSON `metadata` 和 `created_at`。会议级事件覆盖标题、summary、action item 状态和 speaker 修正；全局事件覆盖术语新增、更新和删除。v1 不记录 ASR/LLM 自动生成内容、上传 worker 状态变化或会议删除。
...
- `GET /api/meetings/{meeting_id}/audit-events`：返回指定会议审计事件。
...
`backend/app/api/audit.py` 还提供 `GET /api/audit-events?scope=global&entity_type=glossary_term`，用于本地排障和后续 UI 扩展。
...
`App.tsx` 中选择历史会议后，会把历史 record 作为 `displayedMeeting`，复用同一套 Transcript、Summary、Actions、Analysis 面板展示，并加载该会议的审计事件到 `Audit` tab。标题、summary、action item 和 speaker 编辑成功后会刷新审计列表。全局术语审计事件暂只通过 API 暴露，不放进会议详情工作区。
...
- `activeTab`：当前面板，`transcript`、`summary`、`actions`、`analysis`、`audit`。
...
| `UPLOAD_QUEUE_DIR` | `data/upload_queue` | 上传队列临时 payload 目录 |
| `UPLOAD_QUEUE_EMBEDDED_WORKER_ENABLED` | `true` | 是否随 FastAPI 启动内置上传 worker |
| `UPLOAD_QUEUE_MAX_ATTEMPTS` | `3` | 每个上传任务的队列级最大尝试次数 |
| `UPLOAD_QUEUE_RETRY_BASE_SECONDS` | `30` | 队列级指数退避基础延迟秒数 |
| `UPLOAD_QUEUE_RETRY_MAX_SECONDS` | `300` | 队列级重试最大延迟秒数 |
| `UPLOAD_QUEUE_PROCESSING_TIMEOUT_SECONDS` | `1800` | 启动恢复时判定 processing claim 过期的秒数 |
...
HTTP 请求会经过轻量 observability middleware。后端会接收或生成 `X-Request-ID`，在响应 header 中返回，并通过 context variables 注入日志。日志格式固定包含 `request_id`、`meeting_id`、`job_id`、`provider` 字段，方便把上传 job、worker retry 和 provider 失败串起来排查。

`ObservabilityService` 在进程内维护 request status count 和 provider operation 指标。Provider 指标按 `operation` + `provider` 聚合，记录 count、error count 和 latency 汇总，覆盖 upload ASR、live ASR startup/fallback、翻译、分析、总结和 rolling summary。指标只保存在内存中，后端重启后清零。

`GET /api/diagnostics` 返回服务 uptime、request 计数、provider operation 计数、provider 配置状态和 SQLite 上传队列摘要。队列摘要包含各状态数量、可立即执行的 queued job、延迟重试、processing job、stale processing claim、最老 queued job age 和带 last error 的 job 数量。Diagnostics 不输出 secret、转写文本、绝对音频路径或队列 payload 路径。
...
4. 上传任务使用 SQLite 持久队列；FastAPI 默认启动内置 worker，也可用 `tools/run_upload_worker.py` 独立处理。上传 job 已有有限次数重试、backoff、stale claim 恢复和本地 diagnostics；外部监控和告警仍是后续工作。
5. 队列临时音频和实时临时音频不会作为历史附件长期保存，只保存转写、翻译、总结、分析等结构化结果；用户主动留存的上传原始音频仍单独保存。
6. 编辑审计历史保存在本地 SQLite 中；当前没有账号 actor、审计保留策略或版本恢复 UI。
7. 参与度分析的最终输出是会议级；前端会基于 speaker 发言数估算 participant engagement，但后端 schema 没有输出正式的参与者级分析。
8. ASR、总结、翻译、分析质量依赖第三方模型配置与网络状态。
9. DashScope 路径的 speaker diarization 依赖 pyannote、HuggingFace token、模型加载和本地算力。
